...
public class PodDao {
	...
	*/
	...
	public List<Pod> selectByLabel(Map<String, String> labels, List<Pod> pods) {
		...
		for (Pod pod : pods) { //각각의 pod에 대해 검증 label 검증 
			...
			for (Map.Entry<String, String> polocyLabel : labels.entrySet()) {
				...
				//System.out.println("policyLabel: " + polocyLabel+ "\n");
		...
		this.flag = true;
		//System.out.println("there is return val.");
		//System.out.println("return val: " +  returnList);
		return returnList;
	}
	...
	public List<Pod> selectByPort(String port, List<Pod> pods) {
		...
		if(returnList.isEmpty()) {
			//return null; //123줄 주석 참고
			// //returnList가 pods 전체일 수 있으므로 구분
			if (this.flag == true) {
				return pods;
			}
			else {
	...
	public List<Pod> selectByEgressIngress(IngressEgressPolicy egressIngress, List<Pod> pods) {
		...
		//System.out.println("middd");
		if (egressIngress.getNamespaceSelectorLabel() != null) {
			returnList = selectByLabel(egressIngress.getNamespaceSelectorLabel(), returnList);	
		}
